...
<%@page import="java.util.*,com.pxl.pkb.biz.Pub,java.text.SimpleDateFormat"%>
...
<script type='text/javascript' src='dwr/interface/mms.js'></script>
...
function addrow(tab,taskId){
 if(null!=tab&&''!=tab&&tab!='undefined'){
  ...
  if(tab=='table1'){
    ...
    cell1.innerHTML="<textarea rows='5' cols='40' id='detailContent' name='detailContentadd' />"
...
function closediv(id,taskId){
  if(null!=id&&''!=id&&id!='undefined'){
   ...
   deleteTableNumber('table1',taskId);
   deleteTableNumber('table2',taskId);
   deleteTableNumber('table3',taskId);
  }
}
function deleteTableNumber(tableName,taskId){
 var table = document.getElementById(tableName+taskId);
...
function callbackWorkDetail(data){
      if(null!=data&&''!=data){
       ...
       for(var i=0;i<data.length;i++){
        ...
        cell1.innerHTML="<span onclick=tdClicks('"+i+"','detailContent','fondet','"+data[i].taskID+"')><font id='fondet"+data[i].taskID+i+"'>"+data[i].detailContent+"</font><textarea rows='5' cols='40' id='detailContent"+data[i].taskID+i+"' name='detailContentlist' style='visibility: hidden;display: none;' >"+data[i].detailContent+"</textarea></span>"
  ...
  if(null!=data&&''!=data){
    ...
    for(var i=0;i<data.length;i++){
       ...
       var attachname =data[i].attachname;
        ...
        var cell3 = row.insertCell();
        cell2.setAttribute("width","200");  
        cell0.innerHTML="<span onclick=tdClicks('"+i+"','seqnums','fonseqs','"+data[i].taskID+"')><font id='fonseqs"+data[i].taskID+i+"'>"+data[i].seqNum+"</font><input type=hidden name='outputIdlist"+data[i].taskID+"' id='outputIdlist"+i+"' value='"+data[i].outputID+"'><input value='"+data[i].seqNum+"' type='text' id='seqnums"+data[i].taskID+i+"' name='seqnumslist"+data[i].taskID+"'+ style='width: 100%;height: 100%;visibility: hidden;display: none;'></span>";
        cell1.innerHTML="<span onclick=tdClicks('"+i+"','outputName','fonoutName','"+data[i].taskID+"')><font id='fonoutName"+data[i].taskID+i+"'>"+data[i].outputName+"</font><input value='"+data[i].outputName+"' type='text' id='outputName"+data[i].taskID+i+"' name='outputNamelist"+data[i].taskID+"' style='width: 100%;height: 100%;visibility: hidden;display: none;'></span>";
...
if(null!=data&&''!=data){
    ...
    for(var i=0;i<data.length;i++){
      ...
      var str =new Array();
        ...
        var cell6=row.insertCell();
        var cell7=row.insertCell();
        cell0.innerHTML="<span onclick=tdClicks('"+i+"','problemCode','foncode','"+data[i].taskID+"')><font id='foncode"+data[i].taskID+i+"'>"+data[i].problemCode+"</font><input type=hidden name='problemIdlist"+data[i].taskID+"' id='problemIdlist"+i+"' value='"+data[i].problemID+"'><input value='"+data[i].problemCode+"' type='text' id='problemCode"+data[i].taskID+i+"' name='problemCodelist"+data[i].taskID+"' style='width: 100%;height: 100%;visibility: hidden;display: none;'></span>"
        cell1.innerHTML="<span onclick=tdClicks('"+i+"','description','fondes','"+data[i].taskID+"')><font id='fondes"+data[i].taskID+i+"'>"+data[i].description+"</font><input value='"+data[i].description+"' type='text' id='description"+data[i].taskID+i+"' name='descriptionlist"+data[i].taskID+"' style='width: 100%;height: 100%;visibility: hidden;display: none;'></span>"
        cell2.innerHTML="<span onclick=tdClicks('"+i+"','plan','fonplan','"+data[i].taskID+"')><font id='fonplan"+data[i].taskID+i+"'>"+data[i].plan+"</font><input value='"+data[i].plan+"' type='text' id='plan"+data[i].taskID+i+"' name='planlist"+data[i].taskID+"' style='width: 100%;height: 100%;visibility: hidden;display: none;'></span>"
        cell3.innerHTML="<span onclick=tdClickProblem('"+i+"','charger','foncharger','"+data[i].taskID+"','imgChar')><font id='foncharger"+data[i].taskID+i+"'>"+data[i].chargerName+"</font><input type=hidden name='memberIdlist"+data[i].taskID+"' value='"+data[i].charger+"'  id='memberId"+data[i].taskID+i+"'>  <input value='"+data[i].chargerName+"' type='text' id='charger"+data[i].taskID+i+"' name='chargerlist"+data[i].taskID+"' disabled='disabled' style='width: 60%;height: 100%;visibility: hidden;display: none;'>&nbsp;<img src='images/choose.gif' id='imgChar"+i+"' style='cursor: hand;visibility: hidden;display: none;'  onclick=showDialog('PPMTask.do?method=choseCharge&opercharg=charg&addmody=mody&taskId="+data[i].taskID+"&addrownumber="+i+"','620','290');></span> "
        cell4.innerHTML="<span onclick=tdClickProblem('"+i+"','useroper','fonuseroper','"+data[i].taskID+"','imgOper')><font id ='fonuseroper"+data[i].taskID+i+"'>"+str+"</font><input type='hidden' name='useroperIdlist"+data[i].taskID+"' value='"+data[i].useroperid+"'  id='useroperId"+data[i].taskID+i+"'/><input value='"+str+"' type='text' id='useroper"+data[i].taskID+i+"' name='useroperlist"+data[i].taskID+"' disabled='disabled' style='width: 60%;height: 100%;visibility: hidden;display: none; > &nbsp;<img src='images/choose.gif' style='cursor: hand;visibility: hidden;display: none;' id='imgOper"+i+"'  onclick=showDialog('PPMTask.do?method=choseCharge&opercharg=oper&addmody=mody&taskId="+data[i].taskID+"&addrownumber="+i+"','620','290'); ></span>"
        cell5.innerHTML="<span onclick=tdClicks('"+i+"','planTime','fonplanTime','"+data[i].taskID+"')><font id='fonplanTime"+data[i].taskID+i+"'>"+data[i].planTime+"</font><input value='"+data[i].planTime+"' type='text' id='planTime"+data[i].taskID+i+"' name='planTimelist"+data[i].taskID+"' style='width: 100%;height: 100%;visibility: hidden;display: none;' onfocus=WdatePicker({dateFmt:'yyyy-MM-dd'}); ></span>"
        if(status=='0'){
         cell6.innerHTML="<span onclick=tdClicks('"+i+"','status','fonstatus','"+data[i].taskID+"')><font id='fonstatus"+data[i].taskID+i+"'>已解决</font><input type='hidden' value='"+status+"' name='problemsta"+data[i].taskID+"' id='problemsta' ><select id='status"+data[i].taskID+i+"' name='statuslist"+data[i].taskID+"' style='width: 90%;height: 100%;visibility: hidden;display: none;' ><option value=''></option><option value='0'>已解决</option><option value='1'>未解决</option></select></span>"
        }
        if(status=='1'){
         cell6.innerHTML="<span onclick=tdClicks('"+i+"','status','fonstatus','"+data[i].taskID+"')><font id='fonstatus"+data[i].taskID+i+"'>未解决</font><input type='hidden' value='"+status+"' name='problemsta"+data[i].taskID+"' id='problemsta' ><select id='status"+data[i].taskID+i+"' name='statuslist"+data[i].taskID+"' style='width: 90%;height: 100%;visibility: hidden;display: none;' ><option value=''></option><option value='0'>已解决</option><option value='1'>未解决</option></select></span>"
        }
...
function savaProblem(number,taskId){
   ...
   var problemIdlist = document.getElementsByName("problemIdlist"+taskId);
   var problemCodelist = document.getElementsByName("problemCodelist"+taskId);
   var descriptionlist = document.getElementsByName("descriptionlist"+taskId);
   var planlist = document.getElementsByName("planlist"+taskId);
   var memberIdlist = document.getElementsByName("memberIdlist"+taskId);
   var useroperlist= document.getElementsByName("useroperIdlist"+taskId);
   var planTimelist= document.getElementsByName("planTimelist"+taskId);
   var statuslist = document.getElementsByName("statuslist"+taskId);
   var problemsta = document.getElementsByName("problemsta"+taskId);
...
function saveOutput(number){
  var taskId = document.getElementById("taskId"+number).value;
   ...
   var outputIdlist= document.getElementsByName("outputIdlist"+taskId);
   var outputNamelist= document.getElementsByName("outputNamelist"+taskId);
   var seqnumslist = document.getElementsByName("seqnumslist"+taskId);
...
<tr>
<td height="30"><a href="index.jsp">首页</a>&nbsp;&gt;&nbsp;工作日志</td>
</tr>
</table>
...
<table width="100%" border="0" cellspacing="2" cellpadding="0">
  <tr>
    <td  height="25">任务序号</td>
    <td  height="25">工作任务</td>
    <td height="25" >所在项目</td>
    <td  height="25" width="11%">开始日期</td>
    <td  height="25" width="11%">截止日期</td>
    <td  height="25" width="5%">负责人</td>
    <td  height="25" width="5%">负责人(内部)</td>
    <td  height="25">配合人</td>
    <td height="25" width="5%">完成标志</td>
    <td height="25" align="center" width="6%">操作</td>
  </tr>
	...
	<tr>
  	...
  	if(null!=ppmtask){
  		List list =Pub.getTaskon(ppmtask);
  		int countIndex = 0;
  	for(int i=0;i<list.size();i++){
  			 ppm_taskon task =(ppm_taskon)list.get(i);
  			 %>
  			 <tr 
  			<%if((format.parse(Pub.getCurrTime())).after((format.parse(task.getEndTime())))&&task.getFinishedPercent()<100){
  				%>
  				style="color: red"
  				<%
  			}%> 
  			  >
  	<%
  	 boolean flag =Pub.getTaskon3(ppmtask,task.getTaskID());
  	 ...
  	 if(flag==true){
  		 %>
  		  ...
  		  %>
  		  <img src="images/xtree/Tplus.png" id="imgopen<%=i%>" style="cursor: hand;" onclick="javaScript:opendiv('<%=i%>','<%=task.getTaskID()%>');" />
          <img src="images/xtree/Tminus.png" id="imgclose<%=i%>"  style="visibility: hidden;display: none;cursor: hand" onclick="javaScript:closediv('<%=i%>','<%=task.getTaskID()%>');" >	
	      <img src="images/xtree/new.png">
	      <%=Integer.parseInt(task.getSeqNum())+1%>
	   </td>
  		 <%
  	 }else{
  		 %>
  		 ...
  		 if(task.getParentTaskID()==0){
  			countIndex=0;
  			 %>
	        <img src="images/xtree/root.gif" /><%=Integer.parseInt(task.getSeqNum())+1%>
	      <%
  		 }
  		 for(int j=0;j<countIndex;j++){
  			 if(j+1==countIndex){
  				...
  				<img src="images/xtree/openfoldericon.png">
  				<%=Integer.parseInt(task.getSeqNum())+1%>
  				 <%
   ...
   <% boolean flagpub =Pub.getTaskon4(ppmtask,task.getTaskID());
      if(flagpub==false&&task.getPxlUsername().equals(user.getUserName())&&task.getFinishedPercent()!=100){
    %>
...
String[][] strs =(String[][])request.getAttribute("useroper"); %>
 <table border="1" width="100%" class="tab"  align="left" >
<tr class="TableHeader">
...
<tr class="TableHeader">
  ...
  <%if(null!=fivetask&&!"".equals(fivetask)){
	  for(int i=0;i<fivetask.length;i++){
		  ...
		  boolean flag =Pub.getTaskon3(fivetask,task.getTaskID());
		  if(flag==true){
		  %>
		  <tr>
	    <td  height="25" width="60" ><%=Integer.parseInt(task.getSeqNum())+1%></td>
	    <td  height="25"><%=task.getTaskTitle() %></td>
	    <td  height="25"><%=task.getProjectName()%></td>
	    <td  height="25"><%=task.getStartTime()%></td>
	    <td  height="25"><%=task.getEndTime()%></td>
