...
function uploadSuccess(file, serverData) {
	try {
        ...
        document.getElementById("progressbar"+file.id).style.visibility="hidden";
        document.getElementById("progressbar"+file.id).style.display="none";
		progress.setStatus("&nbsp;&nbsp;&nbsp;<font id='font"+file.id+"'>(上传成功)</font>&nbsp;&nbsp;<img id='imagedelete"+file.id+"' src=\"images/b_drop.png \" onclick=\"deleteUploadFile('"+file.id+"','"+this.customSettings.progressTarget+"');\">");
		progress.toggleCancel(false);
		var doctitle = file.name;
		var titles = doctitle.split(".");
        window.parent.document.getElementById("docTitle").value=titles[0];
	} catch (ex) {
		this.debug(ex);
	}
}
...
function queueComplete(numFilesUploaded) {
	//window.document.getElementById("attachCount").value=numFilesUploaded;
